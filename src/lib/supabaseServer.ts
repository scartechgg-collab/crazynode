...
export function getCookie(request: Request, name: string) {
  ...
  return header.match(new RegExp(`(?:^|;\\s*)${escaped}=([^;]+)`))?.[1] || null;
}

export async function getSupabaseUser(request: Request): Promise<{ user: User; accessToken: string } | null> {
...
export async function getSupabaseAdmin(request: Request) {
  ...
  if ((auth.user.email || "").toLowerCase() !== allowed) return null;
  return auth;
}

export function sessionCookieHeaders(accessToken: string, refreshToken?: string) {
  const secure = process.env.NODE_ENV === "production" ? "; Secure" : "";
  const headers = [
    `sb_access_token=${accessToken}; Path=/; HttpOnly; SameSite=Lax; Max-Age=3600${secure}`,
  ];
  if (refreshToken) {
    headers.push(`sb_refresh_token=${refreshToken}; Path=/; HttpOnly; SameSite=Lax; Max-Age=2592000${secure}`);
  }
  return headers;
}
